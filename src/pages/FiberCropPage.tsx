...
const COLORS = ['#F5F5DC', '#8B4513', '#D2B48C', '#556B2F', '#BDB76B', '#DEB887', '#F4A460', '#DAA520', '#CD853F', '#BC8F8F'];

// FAO ürün kodları ile lif bitkileri
const FIBER_PRODUCTS = [
  { id: '328', name: 'Seed cotton, unginned', nameTR: 'Pamuk' },
  { id: '780', name: 'Jute, raw or retted', nameTR: 'Jüt' },
  { id: '773', name: 'Flax, processed but not spun', nameTR: 'Keten' },
  { id: '777', name: 'Hemp, raw or retted', nameTR: 'Kenevir' },
  { id: '789', name: 'Sisal, raw', nameTR: 'Sisal' },
  { id: '782', name: 'Abaca, manila hemp', nameTR: 'Abaka' },
  { id: '800', name: 'Ramie', nameTR: 'Rami' },
];
...
export default function FiberCropPage() {
  const [selectedProducts, setSelectedProducts] = useState<string[]>(['328', '780', '773']);
    ...
    try {
      ...
      if (productRes.data) {
        const mapped = productRes.data.map((item, index: number) => {
          ...
          const product = FIBER_PRODUCTS.find(p => p.id === urunKod);
          return {
  ...
  return (
    <div>
      <div className="page-header">
        <h1 className="page-title">🧵 Lif Bitkileri Üretimi</h1>
        <p className="page-subtitle">Dünya lif bitkileri üretim miktarları - FAO {selectedYear} (Ton)</p>
      </div>
                ...
                <Treemap
                  ...
                  content={({ x, y, width, height, name, value }: { x: number; y: number; width: number; height: number; name: string; value: number }) => (
                    <g>
                      ...
                      {width > 60 && height > 30 && (
                        <>
                          <text x={x + width / 2} y={y + height / 2 - 8} textAnchor="middle" fill="#333" fontSize={11} fontWeight="bold">{name?.substring(0, 12)}</text>
                          <text x={x + width / 2} y={y + height / 2 + 10} textAnchor="middle" fill="#333" fontSize={10}>{formatNumber(value)}</text>
                        </>
                      )}
                    </g>
                  )}
            ...
            <div className="chart-card">
              ...
              <ResponsiveContainer width="100%" height={300}>
                <ComposedChart data={countryData.slice(0, 10)}>
                  ...
                  <Legend />
                  <Bar yAxisId="left" dataKey="value" name="Üretim" fill="#8B4513" radius={[4, 4, 0, 0]} />
                  <Line yAxisId="right" type="monotone" dataKey="share" name="Pay %" stroke="#556B2F" strokeWidth={2} dot={{ fill: '#556B2F' }} />
                </ComposedChart>
              </ResponsiveContainer>
            </div>
          </div>
            ...
            <div className="chart-card">
              ...
              <ResponsiveContainer width="100%" height={300}>
                <RadarChart data={radarData}>
                  ...
                  <Radar name="Üretim (M ton)" dataKey="value" stroke="#8B4513" fill="#8B4513" fillOpacity={0.5} />
            ...
            <div className="chart-card">
              ...
              <ResponsiveContainer width="100%" height={300}>
                <ScatterChart>
                  ...
                  <Scatter name="Ürünler" data={scatterData} fill="#8B4513">
          ...
          <div className="data-table">
            ...
            {sortedCountryData.map((country, index) => (
              <div className="table-row" key={country.name}>
                <div className={`table-rank ${index < 3 ? 'teal' : ''}`}>{index + 1}</div>
                <div className="table-info">
                  <div className="table-name">{country.name}</div>
                  <div className="table-subtext">Pay: %{country.share}</div>
                </div>
                <div className="table-value teal">{formatTon(country.value)}</div>
              </div>
            ))}
          </div>
        </>
