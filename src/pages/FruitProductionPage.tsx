...
const COLORS = ['#f59e0b', '#3b82f6', '#10b981', '#ef4444', '#8b5cf6', '#ec4899', '#14b8a6', '#f97316', '#06b6d4', '#84cc16'];

const FRUIT_PRODUCTS = [
  { id: 'Apples', name: 'Apples', nameTR: 'Elma' },
  { id: 'Grapes', name: 'Grapes', nameTR: 'Üzüm' },
  { id: 'Oranges', name: 'Oranges', nameTR: 'Portakal' },
  { id: 'Bananas', name: 'Bananas', nameTR: 'Muz' },
  { id: 'Watermelons', name: 'Watermelons', nameTR: 'Karpuz' },
  { id: 'Peaches and nectarines', name: 'Peaches', nameTR: 'Şeftali' },
  { id: 'Pears', name: 'Pears', nameTR: 'Armut' },
  { id: 'Lemons and limes', name: 'Lemons', nameTR: 'Limon' },
  { id: 'Strawberries', name: 'Strawberries', nameTR: 'Çilek' },
  { id: 'Mangoes, guavas and mangosteens', name: 'Mangoes', nameTR: 'Mango' },
  { id: 'Pineapples', name: 'Pineapples', nameTR: 'Ananas' },
  { id: 'Cherries', name: 'Cherries', nameTR: 'Kiraz' },
  { id: 'Plums and sloes', name: 'Plums', nameTR: 'Erik' },
  { id: 'Apricots', name: 'Apricots', nameTR: 'Kayısı' },
  { id: 'Figs', name: 'Figs', nameTR: 'İncir' },
];
...
export default function FruitProductionPage() {
  const [selectedProducts, setSelectedProducts] = useState<string[]>(['Apples', 'Grapes', 'Oranges', 'Bananas']);
    ...
    try {
      ...
      if (productRes.data) {
        ...
        const mapped = productRes.data.map((item: any, index: number) => {
          const product = FRUIT_PRODUCTS.find(p => p.id === item.ürün);
          return {
        ...
        <>
          <div className="kpi-grid">
            <div className="kpi-card large">
              <div className="kpi-header"><div className="kpi-icon orange">💰</div><span className="kpi-title">TOPLAM DEĞER</span></div>
            ...
            <div className="kpi-card">
              <div className="kpi-header"><div className="kpi-icon green">📊</div><span className="kpi-title">ÜRÜN SAYISI</span></div>
              <div className="kpi-value">{productData.length}</div>
              <div className="kpi-subtitle">Aktif ürün</div>
            </div>
            <div className="kpi-card">
              <div className="kpi-header"><div className="kpi-icon purple">🌍</div><span className="kpi-title">ÜRETİCİ ÜLKE</span></div>
              <div className="kpi-value">{countryCount}</div>
              <div className="kpi-subtitle">Top ülkeler</div>
            </div>
            <div className="kpi-card">
              <div className="kpi-header"><div className="kpi-icon blue">🏆</div><span className="kpi-title">LİDER ÜLKE</span></div>
            ...
            <div className="chart-card">
              ...
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={productData}>
                  ...
                  <Tooltip formatter={(value: number) => [formatMoney(value * 1000), 'Değer']} contentStyle={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '8px' }} />
                  <Bar dataKey="value" radius={[4, 4, 0, 0]}>
                    {productData.map((_entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
                    ...
                    <g>
                      ...
                      {width > 60 && height > 30 && (
                        <>
                          ...
                          <text x={x + width / 2} y={y + height / 2 + 10} textAnchor="middle" fill="#fff" fontSize={10}>{formatNumber(value * 1000)}</text>
                        </>
                      )}
                    </g>
                  )}
            ...
            <div className="chart-card">
              ...
              <ResponsiveContainer width="100%" height={300}>
                <ComposedChart data={countryData.slice(0, 10)}>
                  ...
                  <Legend />
                  <Bar yAxisId="left" dataKey="value" name="Değer" fill="#f59e0b" radius={[4, 4, 0, 0]} />
            ...
            <div className="chart-card">
              ...
              <ResponsiveContainer width="100%" height={300}>
                <RadarChart data={radarData}>
                  ...
                  <Radar name="Üretim (M)" dataKey="value" stroke="#f59e0b" fill="#f59e0b" fillOpacity={0.5} />
              ...
              <ResponsiveContainer width="100%" height={300}>
                <ScatterChart>
                  ...
                  <Scatter name="Ürünler" data={scatterData} fill="#f59e0b">
                    {scatterData.map((_entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />))}
                  </Scatter>
                </ScatterChart>
              </ResponsiveContainer>
            </div>
          ...
          <div className="data-table">
            ...
            {sortedCountryData.map((country, index) => (
              <div className="table-row" key={country.name}>
                ...
                <div className="table-info"><div className="table-name">{country.name}</div><div className="table-subtext">Pay: %{country.share}</div></div>
                <div className="table-value orange">{formatMoney(country.value * 1000)}</div>
              </div>
            ))}
          </div>
        </>
