...
const COLORS = ['#8B4513', '#D2691E', '#CD853F', '#DEB887', '#F5DEB3', '#D2B48C', '#BC8F8F', '#F4A460', '#DAA520', '#B8860B'];

// FAO ürün kodları ile baklagiller
const LEGUME_PRODUCTS = [
  { id: '176', name: 'Beans, dry', nameTR: 'Fasulye (Kuru)' },
  { id: '187', name: 'Peas, dry', nameTR: 'Bezelye (Kuru)' },
  { id: '191', name: 'Chick peas', nameTR: 'Nohut' },
  { id: '201', name: 'Lentils', nameTR: 'Mercimek' },
  { id: '181', name: 'Broad beans', nameTR: 'Bakla (Kuru)' },
  { id: '195', name: 'Cow peas', nameTR: 'Börülce' },
  { id: '197', name: 'Pigeon peas', nameTR: 'Güvercin Bezelyesi' },
  { id: '211', name: 'Pulses nes', nameTR: 'Baklagil (Diğer)' },
];
...
export default function LegumeProductionPage() {
  const [selectedProducts, setSelectedProducts] = useState<string[]>(['176', '191', '201']);
    ...
    try {
      ...
      if (productRes.data) {
        const mapped = productRes.data.map((item, index: number) => {
          ...
          const product = LEGUME_PRODUCTS.find(p => p.id === urunKod);
          return {
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p className="page-subtitle">Dünya baklagil üretim miktarları - FAO {selectedYear} (Ton)</p>
      </div>
      ...
      <div className="date-filter">
        <div className="filter-group">
          ...
          <ProductSelector
            products={LEGUME_PRODUCTS}
            selectedProducts={selectedProducts}
            onSelectionChange={setSelectedProducts}
            placeholder="Baklagil seçin..."
          />
        </div>
        <div className="filter-group">
          <label className="filter-label">
