...
export default function LivestockStocksPage() {
  ...
  // Load predictions data (F1.2) - Sprint 5 Intelligence
  const loadPredictionsData = useCallback(async () => {
    ...
    try {
      ...
      // Q1: 15-year historical data for top countries & products
      // Q2: Global yearly aggregates for forecast chart
      const [histRes, globalYearlyRes] = await Promise.all([
      ...
      if (histRes.data) {
        ...
        const forecasts: Array<{ country: string; rawCountry: string; product: string; forecast: Array<{year: number; value: number}>; trend: string; r2: number; historical: YearValue[]; slope: number; stdDev: number }> = [];
        ...
        grouped.forEach((values, key) => {
          ...
          // Calculate stdDev for confidence bands
          ...
          forecasts.push({
            ...
            historical: values, slope: forecastResult.slope, stdDev
          });
        ...
        const upTrend = forecasts.filter(f => f.trend.includes('UP') || f.trend === 'EXPONENTIAL').length;
        const downTrend = forecasts.filter(f => f.trend === 'DECLINING').length;
        ...
        ins.push({ id: `pr${iid++}`, type: avgR2 > 0.7 ? 'achievement' : 'info',
          message: `${forecasts.length} tahmin modeli oluşturuldu. Ortalama R²: ${avgR2.toFixed(3)}. ${highConf} model yüksek güvenilirlikte (R²>0.8).`,
        ...
        <>
                  ...
                  <div className="chart-card" style={{ marginBottom: 24 }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 12 }}>
                      {[
                        ...
                        { tab: 'predictions' as Tab, icon: '🔮', title: 'Tahmin & Öngörü', desc: 'Lineer regresyon, güven bantları, anomali tespiti', color: '#f59e0b' },
          ...
          {activeTab === 'predictions' && (
            <>
              ...
              {predKPIs && (
                <div className="kpi-grid">
                  <div className="kpi-card large">
                    ...
                    <div className="kpi-subtitle">3 yıllık regresyon ({parseInt(selectedYear) + 1}-{parseInt(selectedYear) + 3})</div>
                  </div>
                  <div className="kpi-card">
              ...
              <div className="chart-grid" style={{marginTop: '20px'}}>
                <div className="chart-card" style={{gridColumn: 'span 2'}}>
                  <h3 className="chart-title">🔮 Küresel Üretim Tahmini (Tarihçe + 3Y Projeksiyon)</h3>
                  <ResponsiveContainer width="100%" height={360}>
                  ...
                  <div style={{display: 'flex', gap: '16px', justifyContent: 'center', marginTop: '8px', fontSize: '12px', color: 'var(--text-secondary)'}}>
                    ...
                    <span><span style={{display: 'inline-block', width: '12px', height: '8px', background: 'rgba(34,197,94,.2)', marginRight: '4px', verticalAlign: 'middle'}} /> Güven Bandı</span>
                  </div>
                </div>
              </div>
              ...
              <div className="chart-grid" style={{marginTop: '20px'}}>
                <div className="chart-card">
                  <h3 className="chart-title">📐 Model Güvenilirliği × Büyüme Oranı</h3>
                  <ResponsiveContainer width="100%" height={360}>
                    <ScatterChart>
                      <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
                      ...
                      return (
                        <div key={i} style={{background: isUp ? 'rgba(34,197,94,.06)' : 'rgba(239,68,68,.06)', border: `1px solid ${isUp ? 'rgba(34,197,94,.3)' : 'rgba(239,68,68,.3)'}`, borderRadius: '12px', padding: '14px'}}>
                          ...
                          <div style={{marginTop: '8px', fontSize: '11px', color: 'var(--text-secondary)'}}>
                            Trend: <span style={{fontWeight: 600, color: tf.trend.includes('UP') || tf.trend === 'EXPONENTIAL' ? '#22c55e' : tf.trend === 'DECLINING' ? '#ef4444' : '#f59e0b'}}>
                              {tf.trend === 'EXPONENTIAL' ? '🚀 Üstel Artış' : tf.trend === 'LINEAR_UP' ? '📈 Lineer Artış' : tf.trend === 'DECLINING' ? '📉 Düşüş' : '➡️ Stabil'}
                            </span>
                          </div>
                        </div>
                      );
                        ...
                        <tr key={idx} style={{borderBottom: '1px solid var(--border)'}}>
                          ...
                          <td style={{padding: '6px', textAlign: 'right'}}>
                            <span style={{fontSize: '10px', fontWeight: 700, padding: '2px 6px', borderRadius: '4px',
                              ...
                              color: pred.trend.includes('UP') || pred.trend === 'EXPONENTIAL' ? '#22c55e' : pred.trend === 'DECLINING' ? '#ef4444' : '#f59e0b'}}>
                              {pred.trend === 'EXPONENTIAL' ? '🚀' : pred.trend.includes('UP') ? '📈' : pred.trend === 'DECLINING' ? '📉' : '➡️'} {pred.trend}
                            </span>
                          </td>
