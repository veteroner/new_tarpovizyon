...
export function VeriKarti({ kart, renk }: { kart: Kart; renk: string }) {
  ...
  return (
    <button
       ...
       * duruyordu (ölçüldü).
       */
      className="group flex h-full min-h-[196px] w-full flex-col rounded-[18px] border border-[var(--tv-cizgi-ince)]
    ...
    >
      <span className="flex items-start gap-2">
        <span
          ...
          aria-hidden="true"
        />
        <span className="text-[13px] font-medium uppercase leading-tight tracking-[0.08em] text-[var(--tv-ikincil)]">
          {kart.etiket}
        </span>
        <ArrowUpRight
          size={15}
      ...
      <span className="mt-3 flex items-baseline gap-1.5 tabular-nums">
        <span className="text-[2.5rem] font-semibold leading-none tracking-[-0.03em] text-[var(--tv-murekkep)]">
          {sayi}
        </span>
        {birim && <span className="text-[15px] text-[var(--tv-ikincil)]">{birim}</span>}
      </span>

      <span className="mt-1 text-[14px] text-[var(--tv-ikincil)]">{kart.alt}</span>
