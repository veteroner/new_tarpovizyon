...
export function PlantExportDetailPage() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      let detailQuery = queries.plantExportDetail;
      ...
      // Summary data loaded for potential future use
    } catch (error) {
  ...
  // Year-over-year comparison (uses raw values)
  const yearlyTotals = useMemo(() => {
    ...
    return Array.from(yearMap.entries())
      .map(([year, totals]) => ({
        year,
        totalValue: totals.deger,
        totalQuantity: totals.miktar,
        avgUnitPrice: totals.miktar > 0 ? totals.deger / totals.miktar : 0,
      }))
      .sort((a, b) => b.year.localeCompare(a.year));
  }, [filteredData]);

  const latestYear = yearlyTotals[0];
  const previousYear = yearlyTotals[1];

  // Top product → country combinations (aggregated)
  const topCombinations = useMemo(() => {
    ...
    filteredData.forEach(d => {
      ...
      const current = pairMap.get(key) || {
        ...
        toplam_deger: 0,
        toplam_miktar: 0,
        birimler: new Set<string>(),
        yil_set: new Set<string>(),
      };

      current.toplam_deger += d.deger;
      current.toplam_miktar += d.miktar;
      if (d.birim) current.birimler.add(d.birim);
      if (d.yil) current.yil_set.add(d.yil);
    ...
    return Array.from(pairMap.values())
      .map(v => ({
        ana_urun: v.ana_urun,
        ulke: v.ulke,
        toplam_deger: v.toplam_deger,
        toplam_miktar: v.toplam_miktar,
        birim: v.birimler.size === 1 ? Array.from(v.birimler)[0] : (v.birimler.size > 1 ? 'çeşitli' : ''),
        yil_sayisi: v.yil_set.size,
        birim_fiyat: v.toplam_miktar > 0 ? (v.toplam_deger / v.toplam_miktar) : 0,
      }))
  ...
  const combinationColumns = [
    ...
    },
    {
      ...
      label: 'Toplam Miktar',
      align: 'right' as const,
      formatter: (val: number, row: any) => `${formatNumber(val)} ${row.birim || ''}`,
    },
    {
      key: 'birim_fiyat',
      label: 'Birim Fiyat',
  ...
  return (
    <div>
      ...
      <div className="page-header">
        <h1 className="page-title">🌾 Bitkisel Ürünler İhracat Detay Analizi</h1>
        <p className="page-subtitle">
          Ürün bazında detaylı ihracat verileri, ülkeler ve yıllık trendler
        </p>
      </div>
      ...
      <div className="chart-grid">
        ...
        <div className="chart-card">
          <h3 className="chart-title">En Çok İhraç Edilen Ürünler (Top 10)</h3>
        ...
        <div className="chart-card">
          <h3 className="chart-title">En Çok İhracat Yapılan Ülkeler (Top 10)</h3>
          <ResponsiveContainer width="100%" height={350}>
        ...
        <div className="chart-card" style={{ gridColumn: 'span 2' }}>
          <h3 className="chart-title">Yıllık İhracat Trendi</h3>
          <ResponsiveContainer width="100%" height={350}>
