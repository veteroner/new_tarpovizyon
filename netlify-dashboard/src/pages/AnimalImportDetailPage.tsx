...
export function AnimalImportDetailPage() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      let detailQuery = queries.animalImportDetail;
  ...
  // Product-Country matrix (top combinations)
  const topCombinations = useMemo(() => {
    return filteredData
      .map(d => ({
        combination: `${d.ana_urun} → ${d.ulke}`,
        product: d.ana_urun,
        country: d.ulke,
        value: d.deger,
        quantity: d.miktar,
        unitPrice: d.birim_fiyat,
        year: d.yil,
      }))
      .sort((a, b) => b.value - a.value)
      .slice(0, 20);
  ...
  const combinationColumns = [
    { key: 'product', label: 'Ürün', align: 'left' as const },
    { key: 'country', label: 'Ülke', align: 'left' as const },
    { key: 'year', label: 'Yıl', align: 'center' as const },
    {
      key: 'value',
      label: 'Toplam Değer',
      align: 'right' as const,
  ...
  const { latestYear, previousYear } = useMemo(() => {
    ...
    for (const row of filteredData) {
      const year = String(row.yil || '').trim();
      if (!year) continue;
      const current = yearMap.get(year) ?? { totalValue: 0, totalQuantity: 0 };
      yearMap.set(year, {
        totalValue: current.totalValue + (Number(row.deger) || 0),
        totalQuantity: current.totalQuantity + (Number(row.miktar) || 0),
      });
    }
    ...
    const makeYearSummary = (year?: string) => {
      if (!year) return null;
      const item = yearMap.get(year);
      if (!item) return null;
      const avgUnitPrice = item.totalQuantity > 0 ? item.totalValue / item.totalQuantity : 0;
      return {
        year,
    ...
    return {
      latestYear: makeYearSummary(latest),
      previousYear: makeYearSummary(prev),
    };
  }, [filteredData]);
  ...
  return (
    <div>
      ...
      <div className="page-header">
        <h1 className="page-title">🐄 Hayvansal Ürünler İthalat Detay Analizi</h1>
        <p className="page-subtitle">
          Hayvansal ürün bazlı detaylı ithalat verileri, kaynak ülkeler ve yıllık trendler
        </p>
      </div>
